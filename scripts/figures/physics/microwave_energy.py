"""Reproduce the microwave energy calculations and figure."""
...
from blog_reproducibility.physics.microwave_energy import example_payload
from blog_reproducibility.physics.microwave_energy_figure import render_photons_and_power_figure
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_FIGURE_OUTPUT_DIR / "physics",
    ...
    if not args.dry_run:
        payload["figures"] = describe_figures(
            (render_photons_and_power_figure(output_dir=args.output_dir),)
        )
