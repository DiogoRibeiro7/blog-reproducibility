"""Reproduce the Solow steady-state calculation and figure."""
...
from blog_reproducibility.economics.solow_growth import example_payload
from blog_reproducibility.economics.solow_growth_figure import render_steady_state_figure
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_FIGURE_OUTPUT_DIR / "economics",
    ...
    if not args.dry_run:
        payload["figures"] = describe_figures(
            (render_steady_state_figure(output_dir=args.output_dir),)
        )
