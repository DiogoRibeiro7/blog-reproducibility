"""Reproduce the small-count interval coverage, the zero-event bounds and the figure."""
...
from blog_reproducibility.statistics.small_count_intervals import example_payload
...
def main() -> None:
    ...
    if not args.dry_run:
        payload["figures"] = describe_figures(
            (render_small_count_figure(output_dir=args.output_dir, summary=summary),)
        )
